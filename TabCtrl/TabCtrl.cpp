...
HTAB TabCtrl::AddEx(HWND wnd, TCHAR const *text, int image) throw(std::bad_alloc)
{	
	HTAB t_Current = GetFirstEnableTab();

	while(t_Current != NULL)
	{
		if(::GetDlgCtrlID(GetTabWnd(t_Current)) > ::GetDlgCtrlID(wnd))
			return Insert(t_Current,wnd,text,image);

		t_Current = GetNextEnableTab(t_Current);
	}
...
/////////////////////////////////////////////////////////////////////////////
//
COLORREF TabCtrlStyle_VS2008_client_blue::GetCtrlAreaBackColor(TabCtrl * /*pCtrl*/)
{	return RGB(249,249,251);
}
