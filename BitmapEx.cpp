...
bool CBitmapEx::Load(LPCTSTR lpszBitmapFile)
{
	...
	if (lpszBitmapFile != NULL)
	{
		// Open .BMP file
		FILE* file;
		if (!_tfopen_s(&file, lpszBitmapFile, _T("rb")))
		{
			// Deinit members
			if (m_lpData != NULL)
			{
...
void CBitmapEx::Save(LPCTSTR lpszBitmapFile)
{
	// Check for valid .BMP file path
	if (lpszBitmapFile != NULL)
	{
		...
		// Open .BMP file
		FILE* file;
		if (!_tfopen_s(&file, lpszBitmapFile, _T("wb")))
		{
